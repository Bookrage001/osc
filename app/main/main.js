...
if (settings.cli) {
    ...
    app.on('ready',function(){
        launcher = require('./electron-window')({address:address, shortcuts:true, width:680, height:(100 + 8*3 + 29 * Object.keys(settings.options).filter(x => settings.options[x].launcher !== false).length), node:true, color:'#283143'})
    })
