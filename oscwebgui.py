...
class oscRouter(object):
    def __init__(self, port=3334, target='127.0.0.1',pathPatch=None, paramPatch=None):
        self.port = port
        self.target = target.split(' ')
    ...
    def routeosc(self, path, args):
            ...
            target_path, target_port = self.pathPatch[strip]
            ...
            for i in range(len(self.target)):
              _liblo.send('osc.udp://'+self.target[i]+':'+str(target_port), target_path +target_param, args[1])            
