...
function parsetabs(TABS,parent,sub) {
    ...
    $.each(TABS,function(tabId,tabData){
        ...
        if (tabData.tabs) {
            ...
            $.each(tabData.strips,function(stripId,stripData){// parse strips
                var color = stripData.color?'style="background-color:'+stripData.color+'"':'';
                var title = stripData.title || stripId;
                var $strip = $('\
                    <div class="strip">\
                        <div class="title" '+color+'>'+title+'</div>\
                        <div class="fader-wrapper"><div class="fader locked" path="'+stripData.path+'" name="'+stripId+'"></div></div>\
                        <input value="0"></input>\
                    </div>\
                ');
                $strip.fader = $strip.find('.fader')
                
                var range = stripData.range=='db'?{'min': -70,'20%': -40,'45%': -20,'60%': -10,'max': 6}:stripData.range;
                
                var pips = function() {
                    if (stripData.range=='db') {
                ...
                $strip.find('.fader')
                    ...
                    .Link('lower').to($strip.find('input'))
                    .on('slide',function(){
                        if ($(this).hasClass('locked')) return;
                        var p = stripData.path,
                            n = stripData.args,
                            t = stripData.target?stripData.target:defaultTarget,
                            v = stripData.scale1?
                    ...
                    __strips__.push($strip)
                
                $tabContent.append($strip);
            });// strips parsed
...
// Synchronize strips that share the same name 
$(".fader").each(function(){
